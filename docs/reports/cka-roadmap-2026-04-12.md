...
`cka-011` through `cka-032` are now promoted facilitator packs, and `cka-033+` remain roadmap-only candidates.

## Recommended Candidate Packs

| Proposed pack | Focus | Why it matters | Runtime risk | Recommendation |
|---|---|---|---|---|
| `cka-033` | backlog candidate to be defined | Choose the next uncovered deterministic operator workflow after the probe diagnostics drill lands. | TBD | Backlog definition needed |

## Proposed Build Order

1. Define the next `cka-033` candidate from the remaining uncovered operator workflows

## Suggested Problem Shapes

### Next candidate to be defined after `cka-032`

- The next drill after `cka-032` should target one uncovered operator workflow that still fits the deterministic single-domain model.
...
A `cka-033+` drill should be considered ready only when all of the following are true:
