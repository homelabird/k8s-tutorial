...
## Question 604: container runtime and CRI endpoint diagnostics
...
mkdir -p /tmp/exam/q604
cat <<'EOF_CHECKLIST' > /tmp/exam/q604/runtime-diagnostics-checklist.txt
Kubelet Wiring
...
kubectl get configmap runtime-diagnostics-brief -n runtime-lab -o yaml > /tmp/exam/q604/runtime-diagnostics-brief.yaml
```
...
- `/tmp/exam/q604/runtime-diagnostics-checklist.txt` contains the required sections and exact runtime troubleshooting commands
- `/tmp/exam/q604/runtime-diagnostics-brief.yaml` exports the repaired manifest
