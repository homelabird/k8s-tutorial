...
These drafts cover the next five recommended packs from the `cka-014+` roadmap:
...
5. etcd backup and restore workflow
...
- Questions `401` (`Gateway API`), `402` (`logs/resource triage`), `403` (`kubeadm lifecycle planning`), `404` (`CRD/operator checks`), and `405` (`etcd backup/restore workflow`) are fully scaffolded with `answers.md`, `scripts/setup/`, and validation scripts.
...
- Question 405 should stay planning-focused: validate exact `etcdctl snapshot save` and `snapshot restore` commands plus evidence export without attempting a live restore.
...
- `q405` -> `facilitator/assets/exams/cka/018`
