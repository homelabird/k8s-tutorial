## Question 2301: Ephemeral containers and kubectl debug diagnostics
...
mkdir -p /tmp/exam/q2301
cat <<'EOF_CHECKLIST' > /tmp/exam/q2301/debug-diagnostics-checklist.txt
Pod Inventory
...
kubectl get configmap debug-diagnostics-brief -n debug-lab -o yaml > /tmp/exam/q2301/debug-diagnostics-brief.yaml
```
...
- `/tmp/exam/q2301/debug-diagnostics-checklist.txt` contains the required sections and exact pod-debug troubleshooting commands
- `/tmp/exam/q2301/debug-diagnostics-brief.yaml` exports the repaired manifest
