## Question 1101: CronJob schedule, suspend, and history diagnostics
...
mkdir -p /tmp/exam/q1101
cat <<'EOF_CHECKLIST' > /tmp/exam/q1101/cronjob-diagnostics-checklist.txt
CronJob Inventory
...
kubectl get configmap cronjob-diagnostics-brief -n cronjob-lab -o yaml > /tmp/exam/q1101/cronjob-diagnostics-brief.yaml
```
...
- `/tmp/exam/q1101/cronjob-diagnostics-checklist.txt` contains the required sections and exact inventory, schedule, suspend, concurrency, history, and job template inspection steps
- `/tmp/exam/q1101/cronjob-diagnostics-brief.yaml` exports the repaired manifest
