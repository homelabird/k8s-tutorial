## Question 1601: ServiceAccount identity and projected token diagnostics
...
mkdir -p /tmp/exam/q1601
cat <<'EOF_CHECKLIST' > /tmp/exam/q1601/identity-diagnostics-checklist.txt
...
kubectl get configmap identity-diagnostics-brief -n identity-lab -o yaml > /tmp/exam/q1601/identity-diagnostics-brief.yaml
```
...
- `/tmp/exam/q1601/identity-diagnostics-checklist.txt` contains the required sections and exact deployment inventory and workload identity troubleshooting commands
- `/tmp/exam/q1601/identity-diagnostics-brief.yaml` exports the repaired manifest
