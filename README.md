...
The matching self-hosted workflow is `.github/workflows/cka-2026-single-domain-nightly.yml`. It runs a balanced lane matrix from `cka-006` through `cka-051` with `max-parallel: 1` so fixed-port Podman resources do not overlap on the same runner host.
