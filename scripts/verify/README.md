...
- `cka-2026-single-domain-contract-smoke.sh`
  - Verifies that the promoted single-domain `cka-006` through `cka-025` drills stay aligned across the runner inventory, facilitator `labs.json`, facilitator README listing, and promoted template docs without starting the Podman stack.
...
- `run-cka-2026-single-domain-drills.sh`
  - Runs the promoted single-domain `cka-006` through `cka-025` drills sequentially, resetting the local Podman stack and solving each drill through the shared jumphost before evaluation.
...
```bash
./scripts/verify/run-cka-2026-single-domain-drills.sh cka-025
```
